---
title: "GCC /G++ 分别用于编译C及C++源代码"
date: "2011-09-19"
slug: "6789965"
tags: ["#gcc", "#c", "#c++", "#编译器", "#string", "#汇编"]
...
你和我有同感，改掉它,哈哈  
例子用法  
gcc -o hello.exe hello.c (哦,[windows](http://www.2cto.com/os/windows/)用习惯了)  
...
\-C  
在预处理的时候,不删除注释 信息 ,一般和-E使用,有时候分析程序，用这个很  
方便的  
...
\-Wa,option  
此选项传递option给汇[编程](http://www.2cto.com/kf)序;如果option中间有逗号,就将option分成多个选  
项,然后传递给会汇编程序  
...
quit                    退出当前程序   
finsh                  直接执行完   
setp                 这个和next 不同的地方是可以进入函数内部  
until                 直接运行到某行另在编译时 请使用gc -g -oo 不打开最优化,因为优化可能对调试不利,   
1.8.5 gdb 使用范例  
\-----------------  
清单 一个有错误的C 源程序bugging.c  
\-----------------  
#include   
#include
...
gcc和g++的区别是:  
我们在编译c/c++代码的时候，有人用gcc，有人用g++，于是各种说法都来了，譬如c代码用gcc，而c++代码用g++，或者说编译用gcc，链 接用g++，一时也不知哪个说法正确，如果再遇上个extern "C"，分歧就更多了，这里我想作个了结，毕竟知识的目的是令人更清醒，而不是更糊涂。
...
”return-statement with no value“；分别对应前面红色标注的部分。可见C++的语法规则更加严谨一些。

2.编译阶段，g++会调用gcc，对于c++代码，两者是等价的，但是因为gcc命令不能自动和C＋＋程序使用的库联接，所以通常用g++来完成链接，为了统一起见，干脆编译/链接统统用g++了，这就给人一种错觉，好像cpp程序只能用g++似的。

**误区二:gcc不会定义\_\_cplusplus宏，而g++会**
...
＋＋＋＋＋＋＋＋＋＋＋＋＋＋＋＋＋＋＋＋＋＋＋＋＋＋＋＋  
我自己的总结：  
1> 在链接阶段，gcc -lstdc++等价于g++，如果程序使用了C++的库，不加-lstdc++就会出现很多报告说undefined的链接错误。  
2>一条避免犯错的办法就是：**编译链接c程序用gcc；编译链接c++程序用g++**
