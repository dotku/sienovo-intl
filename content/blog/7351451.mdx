...
(3) 总结：这儿的原子操作只保证 decl 执行的原子性，不能保证整个原子操作函数执行的原子性。这个函数的意义就是在保证对操作数加 1后检查其结果是否为 0 （检查且仅检查加 1 操作后的值） , 那么按照这样的思路，不管后面是否有中断改变这个值，跟这里的检测是没有任何逻辑上的关系的，所以其调用者得到的结果还是正确的。

### 2.   volatile 分析用 volatile 关键字声明的变量 i 每一次被访问时，执行部件都会从 i 相应的内存单元中取出 i 的值。
