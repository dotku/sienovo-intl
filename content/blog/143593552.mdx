...
### 一，PCIE协议-switch相关知识点在如何选型PCIe switch之前，需要先熟悉以下PCIe Base协议中关于switch的知识点讲解：

#### 1，什么是switch：协议定义 Switch: A defined System Element that connects two or more Ports to allow Packets to be routed from one Port to another. To configuration software, a Switch appears as a collection of virtual PCI-to-PCI Bridges.
...
下面以PEX8733来讲解DMA：如下图9，说明了DMA操作序列为一个单一通道。在这个例子中，DMA engine搬运数据从主RAM到一个终端。CPU只在步骤1、2和6中使用；其余步骤由DMA引擎执行。
