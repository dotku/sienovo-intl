...
Mediator模式面向对象 设计追求的目标是高内聚、低耦合，最终是为了开发出易维护、易扩展、可复用的软件。所以我们在进行软件概要设计、搭建软件 架构 时要有意识地降低各个类间的耦合性，减少类间的交互。当我们刚开始接触面向对象开发时，由于知识水平以及经验的欠缺，总是以实现功能为第一目标，而忽视了对软件结构的考虑。在代码中总是充斥着太多A类头文件中包含B类指针，B类头文件中又包含A类指针，类似这种操作将导致A类和B类的耦合性太强，我们需要减少A类和B类的交互，Mediator模式可以帮助我们降低类间的耦合性。
